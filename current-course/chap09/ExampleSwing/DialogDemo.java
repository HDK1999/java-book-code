...
public class DialogDemo extends JPanel {
	...
	public DialogDemo(JFrame frame) {
		...
		tabbedPane.addTab("Simple Modal Dialogs", null, frequentPanel,
				simpleDialogDesc);
		tabbedPane.addTab("More Dialogs", null, featurePanel, moreDialogDesc);
		tabbedPane.addTab("Dialog Icons", null, iconPanel, iconDesc);
	...
	private JPanel createSimpleDialogBox() {
		...
		radioButtons[3] = new JRadioButton(
				"Yes/No/Cancel (in the programmer's words)");
	...
	/*
	 * These dialogs are implemented using showMessageDialog, but you can
	 * specify the icon (using similar code) for any other kind of dialog, as
	 * well.
	 */
		...
		showItButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (command == pickOneCommand) {
					...
					CustomDialog dialog = new CustomDialog(frame, "geisel",
							DialogDemo.this);
					dialog.pack();
					dialog.setLocationRelativeTo(frame);
					dialog.show();
